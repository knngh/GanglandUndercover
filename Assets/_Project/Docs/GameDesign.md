...
- 4 人档仅用于本地预览和快速烟测，正式联机推荐 5-10 人
- 内鬼 ≥8 人局启用
- 警察数 ≥ 黑帮数（保证任务进度）
- 每局至少 1 个卧底
...
| 职业 | 能力 1 | 能力 2 | 信息目标 |
|------|--------|--------|---------|
| **UndercoverAgent（卧底）** | 潜伏破坏 | 情报达标后公开背叛 | 在黑帮掩护和警方收网之间选择时机 |
...
| 职业 | 能力 1 | 能力 2 | 独特目标 |
|------|--------|--------|---------|
| **Mole（内鬼）** | 破坏冷却缩短（0.9x） | 情报达标后锁定卧底 | 窃取 Police 任务进度，完成定点清除 |
...
- 真黑帮可以尝试击杀除自己外的任意存活玩家，误杀真黑帮或内鬼会照常生效。该风险用于防止“击杀按钮是否亮起”成为识别卧底/内鬼的身份探测器。
- 内鬼没有普通击杀权；情报达标后只可击杀服务端私下分配的卧底目标。
- 卧底不能击杀。其进攻手段是潜伏破坏、证据收集、会议推理和达标后的公开背叛。
...
- 每次会议由所有存活玩家参加并投票，包含 Police、Gang、Undercover 和 Mole；死亡玩家只能进入鬼魂聊天，不能影响投票。
- 投票阶段只显示“已提交人数”，不实时公开票向；结算时只公开聚合票数，避免最后投票者获得不对称的跟票优势。
- 会议聊天和玩家外观只显示表面身份。双面身份只有本人、服务端以及结算后的玩家可见。
